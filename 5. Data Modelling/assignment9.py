...
X_train, X_test, y_train, y_test = train_test_split(roomBoard, accept, test_size=0.30, random_state=7 )
...
#
# .. your code here ..
f_ug = X.loc[:, "F.Undergrad"]
accept = X.loc[:, "Accept"]

X_train, X_test, y_train, y_test = train_test_split(f_ug, accept, test_size=0.30, random_state=7 )
X_train = X_train.reshape(-1,1)
X_test = X_test.reshape(-1,1)
...
roomBoard_enroll= X.loc[:, ["Room.Board", "Enroll"]]
accept = X.loc[:, "Accept"]

X_train, X_test, y_train, y_test = train_test_split(roomBoard_enroll, accept, test_size=0.30, random_state=7 )
