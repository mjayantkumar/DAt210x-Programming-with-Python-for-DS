...
Plot2D(T, "Reduced dataframe uing PCA.", 1,2, num_to_plot = 40)
...
Plot2D(M, "Reduced dataframe using Isomap.", 1,2, num_to_plot=40)
...
def Plot3D(T, title, x, y, z, num_to_plot=40):
    ...
    ax.set_xlabel('Component: {0}'.format(x))
    ax.set_ylabel('Component: {0}'.format(y))
    ax.set_zlabel('Compenent: {0}'.format(z))
    ax.scatter(T[:,x],T[:,y], T[:, z],marker='.',alpha=0.7)


Plot3D(T, "Reduced dataframe uing PCA.", 0,1,2, num_to_plot = 40)  
Plot3D(M, "Reduced dataframe using Isomap." ,0, 1,2, num_to_plot=40)
plt.show()